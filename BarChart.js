...
export default class BarChart extends Component {
    ...
    disposeData = async (init=false,prevDataY=false)=>{
        ...
        let dataTransformed = this.props.dataY//this.props.dataY.map(v=>Math.round(100-(v*80/max)))
    ...
    render() {
        return (
            ...
            >
                {this.state.dataY.map((v,i)=><AnimatedBar key={"animatedBar-"+Math.random()*100}  prevValue={this.state.prevDataY?this.state.prevDataY[i]:false} value={v} margin={this.state.margin} color={this.state.color} barName={this.state.barName} index={i}/>)}
            </View>
        )
    }
}